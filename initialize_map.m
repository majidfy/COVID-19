...
for i=1:88
 ...
 %Match the dataset with the shapefile which contains the population for each county       
        if strcmp(S(i).COUNTY_CD,count3{j})
...
for i=1:88
     Cases_Table{i}(1,5)=S(i).POP_2010;
...
[env,S_T_loop,I_T_loop]=generate_env(S, grid,limits,Cases_Table)  

save('env','env','Cases_Table','S_T_loop','I_T_loop')
